...
are the self-documenting template). Create it with
`sops stacks/llm/open-webui/secrets.enc.env` (opens `$EDITOR`, encrypts on
save) holding the variable below. The documented decrypt loop renders the
gitignored plaintext at deploy time.
```
...
3. On the NAS: `sudo docker compose -f stacks/llm/docker-compose.yml up
   --build --detach`, then `ps` on the llm and infra stacks — open-webui and
   caddy both healthy.
