...
All services are defined as per-stack Docker Compose projects under `stacks/`
(`infra`, `monitoring`, `media`, `photos`, `sync`), each with its own
`docker-compose.yml` and co-located service config dirs. The `./compose.sh`
wrapper at the repo root drives them; on the NAS the docker actions need root,
so prefix them with `sudo`.

### Bootstrap (fresh host)

1. Install `sops` and `age` (static binaries are fine — they only need to be on
   `PATH`; on Synology drop them in `/usr/local/bin`).
2. Place the shared age private key at `~/.config/sops/age/keys.txt` (macOS:
   `~/Library/Application Support/sops/age/keys.txt`). See [Environment
   Files](#environment-files).
3. Render the plaintext `secrets.env` files from their encrypted sources:

   ```sh
   ./compose.sh decrypt
   ```
4. Create the shared external networks — idempotent, safe to re-run:

   ```sh
   sudo ./compose.sh networks
   ```
5. Build and start every stack (ordered infra → monitoring → photos → media →
   sync):
...
`./compose.sh <action> [stack…]` runs the action against the named stacks, or
against all of them (in order) when none are given. Actions: `up`, `down`,
`pull`, `ps`, `logs`, `build`.

```sh
sudo ./compose.sh up media            # (re)build and start just the media stack
sudo ./compose.sh pull                # pull new images for every stack
sudo ./compose.sh down                # stop every stack (reverse order)
./compose.sh ps monitoring            # status of the monitoring stack
```

Each action maps to a plain `docker compose` invocation against that stack's
file, so you can always drop down to the raw command:

```sh
# equivalent to `sudo ./compose.sh up media`
docker compose -f stacks/media/docker-compose.yml up --build --detach

# stop one stack
docker compose -f stacks/monitoring/docker-compose.yml down

# pull images for one stack
docker compose -f stacks/photos/docker-compose.yml pull
```

To run gallery-dl (profile-gated, exits when done):

```sh
sudo ./compose.sh gallery-dl
```
...
- `.env`: Contains variables we want to reference in every stack's
  `docker-compose.yml` (each stack has a `.env` symlink back to this root file)
  but not add to containers. Useful for extracting common paths and reusable
  values.
- `stacks/<stack>/<service>/secrets.env`: Contains per-service environment
  variables injected into containers using `env_file` in the stack's
  `docker-compose.yml`. Useful for secrets. Git-ignored; generated by
  decrypting `secrets.enc.env` (see below).
  ***Define non-secret environment variables applicable to a single container
  using `environment` in the stack's `docker-compose.yml`.***
- `stacks/<stack>/<service>/secrets.enc.env`: The tracked, encrypted source of
  truth for the service's `secrets.env`. Encrypted with
  [SOPS](https://github.com/getsops/sops) and
  [age](https://github.com/FiloSottile/age) in dotenv mode, so variable
  *names* stay readable in git while *values* are ciphertext.

### Working with encrypted secrets

Every machine that deploys or edits secrets needs the shared age private key
at `~/.config/sops/age/keys.txt` (macOS sops looks in
`~/Library/Application Support/sops/age/keys.txt` instead). The matching
public key is the single recipient in `.sops.yaml`.

- **Decrypt all** (before `./compose.sh up`): `./compose.sh decrypt` renders a
  plaintext `secrets.env` next to every `stacks/*/*/secrets.enc.env`. To do a
  single file by hand:
  `sops decrypt stacks/<stack>/<service>/secrets.enc.env > stacks/<stack>/<service>/secrets.env`
- **Edit in place**: `sops stacks/<stack>/<service>/secrets.enc.env` (opens
  `$EDITOR`, re-encrypts on save). Re-run the decrypt step afterwards so the
  plaintext copy used by compose is current.
- **Add a variable**: edit as above; then document it under the service's
  entry in Special Instructions.
- **New service**: create `stacks/<stack>/<service>/secrets.env`, then
  `sops encrypt --filename-override stacks/<stack>/<service>/secrets.enc.env stacks/<stack>/<service>/secrets.env > stacks/<stack>/<service>/secrets.enc.env`
  (the override makes the `.sops.yaml` creation rule match). Commit only the
  `.enc` file.
...
Caddy requires some secrets to work (`stacks/infra/caddy/secrets.enc.env`):

- `CF_API_TOKEN`: Cloudflare API token used for the DNS-01 TLS challenge.
  Needs Zone.Zone (read) + Zone.DNS (edit) permissions. Generate at
...
- `QBITTORRENT_WEBUI_USERNAME`: Web UI login user.
- `QBITTORRENT_WEBUI_PASSWORD`: PBKDF2 hash of the password.

Generate the password hash using the included script:

```sh
python stacks/media/qbittorrent/gen_password.py
```
...
`stacks/monitoring/grafana/secrets.enc.env` holds:

- `GF_SECURITY_ADMIN_PASSWORD`: Grafana admin password. Applied on first run;
  changing it later requires a Grafana restart. Use single quotes if the
...
`stacks/monitoring/alertmanager/secrets.enc.env` holds:

- `SMTP_FROM`: Gmail address used as the SMTP sender for alert emails.
- `SMTP_PASSWORD`: The Gmail App Password generated above.
- `ALERT_EMAIL_TO`: Email address that receives alert notifications.

### Immich

`stacks/photos/immich/secrets.enc.env` holds:

- `DB_PASSWORD` / `POSTGRES_PASSWORD`: Postgres password, used by
  immich-server and immich-database respectively. Both must be set to the
  same value. Generate with `openssl rand -hex 32`.

#### Version-controlled configuration (`stacks/photos/immich/immich.json`)
...
`stacks/photos/immich/immich.json`. Edit the file and restart `immich-server` to change
them.
...
`stacks/photos/immich/immich.json` → `machineLearning.urls[0]`. If the desktop is offline, Immich
falls back to the NAS-local worker automatically.

**Remote ML workers:**

The NAS (DS1821+) is underpowered for ML. Instead, run ML workers on more capable machines using the compose files in `stacks/photos/immich/`:

- **CPU-only** (macOS / Linux): `stacks/photos/immich/docker-compose.remote-ml.yml`
- **NVIDIA CUDA** (Windows WSL2 / Linux with NVIDIA GPU): `stacks/photos/immich/docker-compose.remote-ml-cuda.yml`

```sh
# CPU-only (e.g. MacBook)
# MacBook CPU: 1-2 workers
MACHINE_LEARNING_WORKERS=2 docker compose -f stacks/photos/immich/docker-compose.remote-ml.yml up -d
...
NVIDIA GPU using `stacks/photos/immich/docker-compose.remote-transcode.yml`
...
2. **Shared storage**, matching `$DATA_ROOT`'s layout — mount the NAS's
   ...
   on Windows/WSL2).
3. **Shared secret.** Copy the NAS's `stacks/photos/immich/secrets.env` to the worker
...
`library.watch.enabled` (`stacks/photos/immich/immich.json`) needs one inotify watch per
...
`stacks/photos/immich/immich.json` - no manual admin-UI steps needed.)
...
Because the stacks are independent Compose projects that must all share these
two Docker networks, the networks are created **once, up front, as external
networks** rather than by any single project — this is what `sudo ./compose.sh
networks` does (idempotent):

```sh
docker network create --driver macvlan --opt parent=eth0 \
  --subnet 192.168.2.0/24 nas_macvlan
docker network create --driver bridge \
  --subnet 172.18.0.0/16 --ip-range 172.18.128.0/17 nas_bridge
```

Each stack's `docker-compose.yml` references them with `external: true` (as
`nas_macvlan` / `nas_bridge`). The `--ip-range 172.18.128.0/17` confines
Docker's dynamic IP allocation to the upper half of the bridge subnet, leaving
`172.18.0.0/17` free for the handful of statically pinned addresses so a
dynamically-placed container can never collide with a pin. Only load-bearing
addresses are pinned: caddy (`192.168.2.3` + `172.18.0.3`), immich-redis
(`172.18.0.10`), and immich-database (`172.18.0.11`); every other container
gets a dynamic IP `≥ 172.18.128.0`.
