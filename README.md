...
each with its own `docker-compose.yml` and co-located service config dirs.

### Bootstrap (fresh host)

1. Install `sops` and `age` (static binaries on `PATH`; on Synology drop them
   in `/usr/local/bin`).
2. Place the shared age private key at `~/.config/sops/age/keys.txt` (see
   [Working with encrypted secrets](#working-with-encrypted-secrets)).
3. Render the plaintext `secrets.env` files as your normal user — root has no
...
6. Build and start every stack, `infra` first:
   ...
   for stack in infra monitoring photos media sync; do
     sudo docker compose -f "stacks/$stack/docker-compose.yml" up --build --detach
   done
   ```
...
```sh
# (re)build and start one stack — also applies a changed service definition
# or a newer pulled image (rebuilds and recreates)
sudo docker compose -f stacks/media/docker-compose.yml up --build --detach

# start a stopped stack without applying any changes
sudo docker compose -f stacks/media/docker-compose.yml up --detach --no-recreate

# pull new images
sudo docker compose -f stacks/photos/docker-compose.yml pull

# status / logs
docker compose -f stacks/monitoring/docker-compose.yml ps
docker compose -f stacks/monitoring/docker-compose.yml logs

# stop one stack
sudo docker compose -f stacks/media/docker-compose.yml down
```

After editing a `secrets.enc.env`, re-render the plaintext before `up`
([Working with encrypted secrets](#working-with-encrypted-secrets)).

To run gallery-dl (profile-gated, exits when done):

```sh
sudo docker compose -f stacks/media/docker-compose.yml --profile gallery-dl up --build gallery-dl
```

## Environment Files

- `.env`: Contains variables we want to reference in every stack's `docker-compose.yml`
  (each stack has a `.env` symlink back to this root file)
  but not add to containers. Useful for extracting common paths and reusable
  values.
- `stacks/<stack>/<service>/secrets.env`: Contains per-service environment variables injected
  into containers using `env_file` in the stack's `docker-compose.yml`. Useful for secrets.
  Git-ignored. Generated by decrypting `secrets.enc.env` (see below).
  ***Define non-secret environment variables applicable to a single container
  using `environment` in `docker-compose.yml`.***
- `stacks/<stack>/<service>/secrets.enc.env`: Tracked, encrypted source of truth for the
  service's `secrets.env`, managed with [SOPS](https://github.com/getsops/sops) and
  [age](https://github.com/FiloSottile/age).

### Working with encrypted secrets

Put the shared age private key (recipient listed in `.sops.yaml`) at
`~/.config/sops/age/keys.txt` and export:
...
- **Decrypt** (before `docker compose up`):
  ...
  ```
- **Edit** (also creates the file for a new service):
  `sops edit stacks/<stack>/<service>/secrets.enc.env`, then re-run the
  decrypt step.
  Document new variables in [Special Instructions](#special-instructions).

## Special Instructions

### Caddy

`stacks/infra/caddy/secrets.enc.env` holds:
...
The host needs the `tun` module for `/dev/net/tun` to exist. Check with
`ls -l /dev/net/tun`; if it is missing, `sudo modprobe tun`.
...
Edits to `stacks/photos/immich/immich.json` apply on `immich-server` restart.

**Before first deploy:** set your desktop worker's LAN IP in
`stacks/photos/immich/immich.json` → `machineLearning.urls[0]`.
...
networks** rather than by any single project:
