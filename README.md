...
- `<service>/secrets.enc.env`: The tracked, encrypted source of truth for the
  service's `secrets.env`. Encrypted with [SOPS](https://github.com/getsops/sops)
  and [age](https://github.com/FiloSottile/age) in dotenv mode, so variable
...
- **Decrypt all** (before `docker compose up`):
  `sops decrypt <service>/secrets.enc.env > <service>/secrets.env`
- **Edit in place**: `sops <service>/secrets.enc.env` (opens `$EDITOR`,
  re-encrypts on save). Re-run the decrypt step afterwards so the plaintext
  copy used by compose is current.
- **Add a variable**: edit as above; then document it under the service's
  entry in Special Instructions.
- **New service**: create `<service>/secrets.env`, then
  `sops encrypt --filename-override <service>/secrets.enc.env <service>/secrets.env > <service>/secrets.enc.env`
...
Caddy requires some secrets to work (`caddy/secrets.enc.env`):
...
Web UI login credentials live in `qbittorrent/secrets.enc.env`:
...
`alertmanager/secrets.enc.env` holds:
