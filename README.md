...
- `<service>/secrets.enc.env`: Encrypted source of truth for the
  service's `secrets.env` managed with [SOPS](https://github.com/getsops/sops)
...
- **Decrypt** (before `docker compose up`):
  ...
  ```sh
  for f in */secrets.enc.env; do sops decrypt "$f" > "${f%.enc.env}.env"; done
  ```

- **Edit** (also creates the file for a new service):
  `sops edit <service>/secrets.enc.env`, then re-run the decrypt step.
